...
router.get('/list', async (ctx, next) => {
    console.log('订单页面请求的页数是', ctx.query)
    let data = Mock.mock({
        "code": 0,
        "result": {
            "page|1-9": 1,
...
router.get('/detail', async (ctx, next) => {
    console.log('请求的详情订单是', ctx.query)
    let data = Mock.mock({
        "code": 0,
        "msg": '',
        "result": {
...
//删除订单
router.get('/ebike_info', async (ctx, next) => {
    console.log('需要删除的订单是', ctx.query)
    let data = Mock.mock({
        "code": 0,
        "result": {
            "id|+1": 1,
...
//删除订单确认
router.get('/finish_order', async (ctx, next) => {
    console.log('需要确认删除的订单是', ctx.query)
    let data = Mock.mock({
        "code": 0
    })
    ctx.body = data
